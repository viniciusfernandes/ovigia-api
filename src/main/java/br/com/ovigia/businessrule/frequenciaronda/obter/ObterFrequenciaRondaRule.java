...
public class ObterFrequenciaRondaRule implements BusinessRule<ObterFrequenciaRondaRequest, ObterFrequenciaRondaResponse> {
  private final ClienteRepository clienteRepository;
  private final CriarFrequenciaRondasBusiness criarFrequenciaRondasBusiness;
  private final VigiaRepository vigiaRepository;

  public ObterFrequenciaRondaRule(ClienteRepository clienteRepository, VigiaRepository vigiaRepository, CriarFrequenciaRondasBusiness criarFrequenciaRondasBusiness) {
  ...
  private Mono<FrequenciaRonda> calcularFrequenciaRonda(String idCliente) {
    ...
    return clienteRepository.obterIdVigiaELocalizacaoByIdCliente(idCliente).flatMap(cliente -> vigiaRepository.obterDataUltimaRonda(cliente.idVigia)).flatMap(vigia -> criarFrequenciaRondasBusiness.apply(idCliente, vigia.dataAtualizacaoRonda));
